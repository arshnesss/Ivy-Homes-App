...
async function fetchAll(endpoint, token) {
  console.log(`Fetching all retrievable records from /v1/${endpoint}...`);
  let limit = 50;
  let offset = 0;
  let allRecords = [];

  while (true) {
    const { status, data } = await req(`/v1/${endpoint}?offset=${offset}&limit=${limit}`, "GET", null, token);
    if (status !== 200 || !data.results) {
      console.error(`Error at offset ${offset}:`, status, data);
      break;
    }
    allRecords.push(...data.results);
    if (!data.has_more || data.results.length === 0 || allRecords.length >= data.total) {
      // Check if more items exist beyond total field (discrepancy check)
...
async function main() {
  ...
  const listings = await fetchAll("listings", token);
  const rentals = await fetchAll("rentals", token);
  const projects = await fetchAll("projects", token);
  ...
  // 2. unique_properties (19 duplicate clusters)
  const sigs = new Set();
  ...
  uniqueListings.forEach(item => {
    ...
    if (lat !== undefined && lng !== undefined) {
      if (lat > 70 && lng < 25) isCorrupt = true; // Swapped
  ...
  // 5. total_monthly_rent for assigned locality 'powai'
  ...
  // 9. fake_listing_ids (11 bait listings)
  const fake_ids = [
  ...
  // 8. listings_last_7_days (Anchored to 2026-09-10T00:00:00+05:30)
  // [2026-09-03T00:00:00+05:30, 2026-09-10T00:00:00+05:30) IST
  ...
  uniqueListings.forEach(item => {
    ...
    if (!pStr.endsWith("Z")) pStr += "Z"; // API returns naive ISO strings in UTC
